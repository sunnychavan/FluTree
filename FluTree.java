package a4;
/* NetId(s): sc2448, tcc86

 * Name(s): Sunny Chavan and Tyler Carreja
 * What I thought about this assignment: We believe that this assignment was a great way of reinforcing 
 * the concepts of trees and recursion. We also were intrigued by mutual recursion. These concepts
 * are important for tech interviews as well.
 */
...
public class FluTree {
	...
	public static double timeSpent= 8.5;
	...
	public FluTree insert(Person c, Person p) throws IllegalArgumentException {
		...
		FluTree parent= node(p);

		if (c == null || p == null || node(c) != null || parent == null) 
			 throw new IllegalArgumentException("Can't construct FluTree");

		FluTree child= new FluTree(c);
	...
	public int size() {
		...
		// Counting method
		if (this.childrenSize() == 0) { return 1; }
		int a= 1;
		for (FluTree S : children) {
			a= a + S.size();
			}
		return a;
		}
	...
	public boolean contains(Person p) {
		...
		//counting method (calls node which is a recursive counting method)
		if (node(p) != null) { return true; }
		return false;
		}
	...
	public int widthAtDepth(int d) throws IllegalArgumentException {
        ...
        for (FluTree child : children) {
        	a= a + child.widthAtDepth(d - 1);
        }
        return a;
    	}
	...
	public Person commonAncestor(Person child1, Person child2) {
		...
		List<Person> a= fluRouteTo(child1);
        List<Person> b= fluRouteTo(child2);
        if(a == null || b == null) return null;
       	
        Person[] array1= new Person[a.size()];
        a.toArray(array1);        
        Person[] array2= new Person[b.size()];
        b.toArray(array2);
	...
	public boolean equals(Object ob) {
		...
		FluTree ft = (FluTree)ob;
		if(root != ft.root) return false;
		if(this.childrenSize() != ft.childrenSize()) return false;
		
		//Mutual Recursive step
		for(FluTree ch:children) {
			if(help(ch, ft.children) == false) return false;
		}
		return true;

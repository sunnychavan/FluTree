...
public class FluTreeTest {
	...
	private FluTree makeTree2() {
		FluTree dt= new FluTree(personA); // A
		dt.insert(personB, personA); // A, B
		dt.insert(personC, personA); // A, C
		dt.insert(personF, personB); // B, D
		dt.insert(personE, personB); // B, E
		dt.insert(personD, personB); // B, F
		dt.insert(personG, personF); // F, G
		dt.insert(personH, personG); // G, H
		dt.insert(personI, personH); // H, I
		return new FluTree(dt);
	...
	public void test1Insert() {
		...
		FluTree st1 = makeTree1();
		Person personq = new Person("Q", 0, n);
		st1.insert(personq, personC);
		assertEquals("A[B[D E F[G[H[I]]]] C[Q]]", toStringBrief(st1));
	...
	public void test2size() {
		...
		int[] size_holder = new int[] {9,7,1,1,1,4,3,2,1};
		Person[] person_holder = new Person[] {personA, personB, personC, personD, personE, personF, 
				personG, personH, personI};
		int k = 0;
		for(Person h: person_holder) {
			assertEquals(size_holder[k], st1.node(h).size());
			k = k + 1;
		}
	}
	...
	public void test3contains() {
		...
		//Case2: more complicated
		FluTree st1 = makeTree1();
		
		boolean[] bool_holder = new boolean[] {true, true, true, true, true, true, true, true, true, 
				false, false, false};
		
		
		Person person_notJ = new Person("J", 0, n);
		Person person_notK = new Person("K", 0, n);
		Person person_notL = new Person("L", 0, n);
		Person[] person_holder = new Person[] {personA, personB, personC, personD, personE, personF, 
				personG, personH, personI, person_notJ, person_notK, person_notL};
		
		int k = 0;
		for(Person h: person_holder) {
			assertEquals(bool_holder[k], st1.contains(h));
			k = k + 1;}
	}
	...
	public void test7commonAncestor() {
		...
		assertEquals(personA, st1.commonAncestor(personA, personB));
		assertEquals(null, st1.commonAncestor(personA, null));
		assertEquals(null, st1.node(personB).commonAncestor(personA, personB));
		
		assertEquals(personB, st1.commonAncestor(personD, personI));
		assertEquals(null, st1.commonAncestor(personA, person_notJ));
	}
	...
	public void test8equals() {
		...
		//Case2 (identical tree)
		FluTree st1 = makeTree1();
		FluTree st2 = makeTree1();
		assertEquals(true, st1.equals(st2));
		
		//Case 3 (non-identical but equal tree)
		FluTree st3 = makeTree2();
		assertEquals(true, st1.equals(st3));
		
		//Case4 (not equal)
		FluTree st4 = makeTree3();
		assertEquals(false, st1.equals(st4));
		assertEquals(false, st3.equals(st4));
